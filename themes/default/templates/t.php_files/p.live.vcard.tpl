<div class="hovercard-inner">
    <div class="bd">
        ...
        <div class="bio">
            ...
            <p class="location">
                <img title="{$tsData.status.t}" class="status {$tsData.status.css} vctip" src="{$tsConfig.tema.t_url}/images/space.gif"/>
                <img title="{if $tsData.user_sexo == 1}Hombre{else}Mujer{/if}" src="{$tsConfig.tema.t_url}/images/icons/{if $tsData.user_sexo == 0}fe{/if}male.png" class="vctip"/>
                <img title="" style="padding:2px" src="{$tsConfig.tema.t_url}/images/flags/{$tsData.user_pais|lower}.png" class="vctip"/>
               <!-- <img title="{$tsData.stats.r_name}" src="{$tsConfig.tema.t_url}/images/icons/ran/{$tsData.stats.r_image}" class="vctip"/> -->
                {if $tsData.p_sitio}<a href="{$tsData.p_sitio}" target="_blank"><img src="{$tsConfig.tema.t_url}/images/icons/www.png" title="Sitio web" class="vctip"/></a>{/if}
                {if $tsUser->uid != $tsData.user_id && $tsUser->is_member}<a onclick="mensaje.nuevo('{$tsData.user_name}','','','');return false" href="#"><img src="{$tsConfig.tema.t_url}/images/icon-mensajes-recibidos.gif" title="Enviar mensaje privado" class="vctip"/></a>{/if}
				{if $tsUser->is_admod == 1}<img title="Administrar" src="{$tsConfig.tema.t_url}/images/icons/editar.png" style="width:14px;height:14px;cursor:pointer;" class="vctip" onclick="location.href = '{$tsConfig.url}/admin/users?act=show&amp;uid={$tsData.user_id}'"/>{/if}
			</p>
        </div>
