...
<div id="div_cmnt_{$tsComment.0}" class="{if $tsComment.4 == $tsUser->uid}especial1{else}especial3{/if}">
    ...
    <div class="none">
                                <div class="col-md-3">
        <div class="troll" style="z-index: 99;">
            <a href="{$tsConfig.url}/perfil/{$tsUser->info.user_name}">
                <img title="{$tsUser->info.user_name}" src="{$tsConfig.url}/files/avatar/{$tsUser->uid}_120.jpg" class="avatar-48 lazy" style="position: relative; z-index: 1; display: inline;"/>
            </a>
        </div></div></div>
        <div class="col-md-9">
        <div class="comment-box">
            ...
            <div class="comment-info clearbeta">
                <div class="floatL">
                    <a href="{$tsConfig.url}/perfil/{$tsUser->nick}" class="nick">{$tsUser->nick}</a>  
                    <time class="time">{$tsComment.3|hace}:</time>
                    {if $tsUser->is_admod} 
                    <a href="{$tsConfig.url}/moderacion/buscador/1/1/{$tsComment.6}" class="ipComentPost" target="_blank">{$tsComment.6}</a>
                    {/if} 
                </div>
                <div style="float:right;margin-top:-21px;" class="floatR answerOptions">
                    <ul>{if $tsComment.0 > 0}
                    	{if $tsUser->is_member}
