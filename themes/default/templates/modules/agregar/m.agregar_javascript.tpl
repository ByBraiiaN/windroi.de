...
$(document).ready(function(){
	...
	$('input[name=preview]').bind('click',function(){
		var error = false;
			...
			if ($('textarea[name=cuerpo]').val().length < 500) {
                showError($('textarea[name=cuerpo]').get(0), 'El Posts es demciado corto para su publicaci&oacute;n.');
                return false;
            }
            if ($('textarea[name=cuerpo]').val().indexOf("kn3.net\/") >= 0) {
				showError($('textarea[name=cuerpo]').get(0), 'El host KN3 para im&aacute;genes no est&aacute; permitido.');
				return false;
			}
			...
			mydialog.body('<div class="text-center">Cargando vista previa....<br><br><img src="' + global_data.url + '/themes/default/images/loading_bar.gif"></div>');
            mydialog.center();
            // PREVIEW
			$.ajax({
				type: 'post',
