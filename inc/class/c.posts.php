...
class tsPosts {
	...
	*/
	function newPost(){
		...
		$postData = array(
			...
			'imgport' => $tsCore->setSecure($_POST['imgport']),
		);
        //ANTIFLOOD
        $antiflood = 2;
        ...
        if(empty($tsUser->is_admod)  && $tsUser->permisos['most'] == false) {
			...
			$postData['blogoff'] = empty($_POST['blogger']) ? 0 : 1;
		...
		if($tsUser->info['user_lastpost'] < (time() - $antiflood)) {
			...
			if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `p_posts` (post_user, post_category, post_title, post_body, post_date, post_tags, post_imgport, post_ip, post_private, post_block_comments, post_blogoff, post_sponsored, post_sticky, post_smileys, post_visitantes, post_status) VALUES (\''.$tsUser->uid.'\', \''.(int)$postData['category'].'\', \''.$postData['title'].'\',  \''.$postData['body'].'\', \''.$postData['date'].'\', \''.$postData['tags'].'\', \''.$postData['imgport'].'\', \''.$_SERVER['REMOTE_ADDR'].'\', \''.(int)$postData['private'].'\', \''.(int)$postData['block_comments'].'\', \''.(int)$postData['blogoff'].'\', \''.(int)$postData['sponsored'].'\', \''.(int)$postData['sticky'].'\', \''.(int)$postData['smileys'].'\', \''.(int)$postData['visitantes'].'\', '.(!$tsUser->is_admod && ($tsCore->settings['c_desapprove_post'] == 1 || $tsUser->permisos['gorpap'] == true) ? '\'3\'' : '\'0\'').')')) {
	...
	*/
	function savePost(){
        ...
        // SOLO MODERADORES Y ADMINISTRADORES
        if(empty($tsUser->is_admod)  && $tsUser->permisos['most'] == false) {
			$postData['blogger'] = $data['post_blogoff'];
		...
		if($tsUser->uid == $data['post_user'] || !empty($tsUser->is_admod) || !empty($tsUser->permisos['moedpo'])){
		    if(db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE p_posts SET post_title = \''.$postData['title'].'\', post_body = \''.$postData['body'].'\', post_tags = \''.$tsCore->setSecure($postData['tags']).'\', post_imgport = \''.$tsCore->setSecure($postData['imgport']).'\', post_category = \''.(int)$postData['category'].'\', post_private = \''.$postData['private'].'\', post_block_comments = \''.$postData['block_comments'].'\', post_blogoff = \''.$postData['blogoff'].'\', post_sponsored = \''.$postData['sponsored'].'\', post_smileys = \''.$postData['smileys'].'\', post_visitantes = \''.$postData['visitantes'].'\', post_sticky = \''.$postData['sticky'].'\' WHERE post_id = \''.(int)$post_id.'\'') or die(mysql_error())) {
	...
	*/
	function getPost(){
	   ...
	   // FECHA
		...
		// DESCRIPCION POST
		$postData['post_desc'] = htmlspecialchars(stripslashes(strip_tags($postData['post_body'])));
		$postData['post_desc'] = preg_replace("[\n|\r|\n\r]", ' ', $postData['post_desc']); 
	...
	*/
	function getLastComentarios(){
		global $tsUser, $tsCore;
		//
		$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT cm.cid, cm.c_status, u.user_name, u.user_activo, u.user_baneado, p.post_id, p.post_category, p.post_title, p.post_status, c.c_seo FROM p_comentarios AS cm LEFT JOIN u_miembros AS u ON cm.c_user = u.user_id LEFT JOIN p_posts AS p ON p.post_id = cm.c_post_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category ORDER BY cid DESC LIMIT 6');
...
global $tsCore;
//
$query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_id, user_name,user_activo FROM u_miembros WHERE user_activo= \'1\' AND user_baneado= \'0\' ORDER BY user_id DESC LIMIT 8');
if(!$query) exit( show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db') );
$data = result_array($query);
...
return $data;
	...
	*/
	function votarPost(){
		...
		$max_points = $tsUser->permisos['gopfp'];
				...
				}else return '0: Voto no v&aacute;lido. No puedes dar '.$puntos.' puntos, s&oacute;lo se permiten '.$max_points .' <img src="https://i.imgur.com/doCpk.gif">';													
    ...
    */
    function getQuery()
    {
		...
		$numero = count($trozos);
		if ($numero == 1) {
			$w_search = "AND p.post_title LIKE '%$q%' OR p.post_body LIKE '%$q%' OR p.post_tags LIKE '%$q%'";
		} elseif ($numero > 1) {		
