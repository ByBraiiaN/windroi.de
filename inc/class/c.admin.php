...
class tsAdmin
{
    ...
    */
    function saveConfig()
    {
        ...
        $c = array(
            ...
            'descripcion' => $tsCore->setSecure($tsCore->parseBadWords($_POST['descripcion'])),
        ...
        if (db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `w_configuracion` SET `titulo` = \'' . $c['titulo'] . '\', `slogan` = \'' .
            $c['slogan'] . '\', `descripcion` = \'' . $c['descripcion'] . '\', `url` = \'' . $c['url'] .
            '\', `cpost` = \'' . $c['cpost'] . '\',`c_last_active` = \'' . $c['active'] . '\', `c_allow_sess_ip` = \'' .
            $c['sess_ip'] . '\', `c_count_guests` = \'' . $c['count_guests'] . '\', `c_reg_active` = \'' .
            $c['reg_active'] . '\', `c_reg_activate` = \'' . $c['reg_activate'] . '\', `c_met_welcome` = \'' .
            $c['met_welcome'] . '\', `c_message_welcome` = \'' . $c['message_welcome'] . '\', `c_hits_guest` = \'' . 
			$c['hits_guest'] . '\', `c_keep_points` = \'' .
            ...
            $c['portal'] . '\', `c_allow_live` = \'' . $c['live'] . '\', `offline` = \'' . 
            $c['offline'] . '\', `offline_message` = \'' . $c['offline_message'] .
            '\' WHERE `tscript_id` = \'1\''))
            return true;
        else
    ...
    */
    ...
    function deleteContent($user_id){
        global $tsUser;
        if( $user_id == 1) return "No puedes eliminar " . ($_POST['bocuenta'] ? "la cuenta" : "contenido") . " del primer administrador";
        ...
        if($_POST['bofavoritos'] || $c) db_exec(array(__FILE__, __LINE__), 'query', 'DELETE FROM p_favoritos WHERE fav_user = \''.$user_id.'\'');
    ...
    */
    ...
    function GetAdminStats()
    {
        ...
        $num['medallas_total'] = $num['medallas_usuarios'] + $num['medallas_posts'];
