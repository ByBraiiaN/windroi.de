...
class tsRegistro{
	...
	function registerUser(){
		...
		$secret = "6LdJIBQUAAAAADgDb3EybyPPTGHk_RFiL5QingcK";
...
// COMPROBAMOS CLONES
            ...
            if($f = db_exec('num_rows', $q = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_name FROM u_miembros WHERE user_last_ip = \''.$tsCore->setSecure($realip).'\'')) || db_exec('num_rows', $qq = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT u.user_name FROM u_sessions AS s LEFT JOIN u_miembros AS u ON s.session_user_id = u.user_id WHERE s.session_ip = \''.$tsCore->setSecure($realip).'\' && s.session_user_id != \'0\''))){
              ...
              while($row = db_exec('fetch_row', $ms)){
                db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO u_avisos (user_id, av_subject, av_body, av_date, av_type) VALUES (\''.$row[0].'\', \'Posibles clones\', \'Posibles clones: @'.$q[0].' y @'.$tsCore->setSecure($tsData['user_nick']).'\', \''.time().'\', \'2\')') or die('0__'.show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db'));
              }
            }
			...
			$body = '<div style="background:#0f7dc1;padding:10px;font-family:Arial, Helvetica,sans-serif;color:#000">
				...
				<div style="background:#FFF;padding:10px;font-size:14px">
					...
					<p>Antes de empezar a interactuar con la comunidad, te recomendamos que visites el <a target="_blank" href="'.$tsCore->settings['url'].'/pages/protocolo/">Protocolo</a> del sitio.</p>
