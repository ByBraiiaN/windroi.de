...
class tsMuro {
    ...
    public function ajaxCheck($return = false, $urlin = null){
        ...
        switch($type){
            ...
            case 'foto':
                ...
                elseif(preg_match('/kn3.net/',$url )) return '0: No se permiten imagenes de KN3';
            ...
            case 'video':
                ...
                // META TAGS
                $data = @get_meta_tags('https://www.youtube.com/watch?v='.$video_id);
                if(empty($data['title'])) return '0: La URL contiene un ID de video incorrecto o el video ha sido eliminado.';
                    ...
                    if($return == false)
                    return '1: <div class="vContent"><img src="https://img.youtube.com/vi/'.$video_id.'/0.jpg" class="thumb"/><div class="vDesc"><strong><a href="https://www.youtube.com/watch?v='.$video_id.'" target="_blank" class="a_blue">'.$data['title'].'</a></strong><div style="margin-top:5px">'.$description.'</div></div><div class="clearBoth"></div></div>';
    ...
    */
    public function streamPost(){
        ...
        $tsMonitor->setFollowNotificacion(17, 1, $tsUser->uid, $pub_id);
        // ACTIVIDAD
    ...
    */
    function getNews($start = 0, $limit = 10){
        ...
        $query = db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.*, u.user_name FROM u_muro AS p LEFT JOIN u_miembros AS u ON p.p_user_pub = u.user_id WHERE p.p_user AND p.p_user = p.p_user_pub ORDER BY p.p_date DESC LIMIT '.$start.','.$limit);
