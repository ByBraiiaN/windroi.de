	...
	*/
    ...
    private function makeMonitor(){
        $this->monitor = array(
			...
			17 => array('text' => 'actualiz&oacute; su', 'ln_text' => 'estado', 'css' => 'blue_ball'),
			// COMUNIDADES
			30 => array('text' => array('cre&oacute; un nuevo'), 'ln_text' => 'tema', 'css' => 'post'),
			31 => array('text' => array('coment&oacute; tu','_REP_ nuevos comentarios en tu'), 'ln_text' => 'tema', 'css' => 'comment_post'),
			32 => array('text' => array('coment&oacute; en un', '_REP_ nuevos comentarios en el'), 'ln_text' => 'tema', 'extra' => ' que sigues', 'css' => 'blue_ball'),
			33 => array('text' => array('respondi&oacute; tu', '_REP_ nuevas respuestas a tu'), 'ln_text' => 'comentario', 'css' => 'comment_resp'),
			34 => array('text' => array('agreg&oacute; a favoritos tu','_REP_ nuevos favoritos a tu'), 'ln_text' => 'tema', 'css' => 'star'),
			35 => array('text' => array('te recomienda un', '_REP_ usuarios te recomiendan un'), 'ln_text' => 'tema', 'css' => 'share'),
			36 => array('text' => array('vot&oacute; _REP_ tu', '_REP_ nuevos votos a tu'), 'ln_text' => 'tema', 'css' => 'voto_'),
			37 => array('text' => array('vot&oacute; _REP_ tu', '_REP_ nuevos votos a tu'), 'ln_text' => 'comentario', 'css' => 'voto_'),
			38 => array('text' => array('est&aacute; siguiendo tu','_REP_ nuevos seguidores a tu'), 'ln_text' => 'tema', 'css' => 'follow'),
        );
    }
	...
	*/
	...
	public function getNotificaciones($unread = false){
		...
		// SI VA AL MONITOR ENTONCES ACTUALIZAMOS PARA QUE YA NO SE VEAN EN EL MENUBAR
            ...
            // DATOS
            $sql = 'SELECT m.*, u.user_name AS usuario FROM u_monitor AS m LEFT JOIN u_miembros AS u ON m.obj_user = u.user_id WHERE m.user_id = \''.$tsUser->uid.'\' ORDER BY m.not_id DESC LIMIT '.$tsCore->settings['c_max_nots'].'';
    ...
    private function makeOracion($data){
        ...
        switch($no_type){
			...
			case 38:
                ...
                $no_total = $data['not_total'];
				$id_comment = '';
				if($no_type == 32) $extra = $this->monitor[$no_type]['extra'];
                // MAS DE UNA ACCION
                if($no_total > 1) {
                    $text = $this->monitor[$no_type]['text'][1].$txt_extra;
                    $oracion['text'] = str_replace('_REP_', "<b>{$no_total}</b>", $text);
                } else $oracion['text'] = ($this->show_type == 1 && $no_type == 32) ? $this->monitor[$no_type]['text'][0].$txt_extra : $this->monitor[$no_type]['text'][0].$txt_extra.$extra;
                // ID COMMENT
                if($no_type == 33 || $no_type == 37){
                    $id_comment = '#coment_id_'.$data['obj_tres'];
                }
				if($no_type == 36 || $no_type == 37){
