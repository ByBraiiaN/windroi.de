...
class BBCode {
     ...
     */
    public function addBBcodes($title) {
        ...
        $tagCodes = array(
            ...
            array('tag' => 'url', 'replace' => '<a rel="nofollow" href="{param}" target="_blank">{param}</a>', 'parse' => false, 'validParam' => $urlValidator),
            array('tag' => 'url', 'replace' => '<a rel="nofollow" href="{option}" target="_blank">{param}</a>', 'option' => true, 'validOption' => $urlValidator),
            array('tag' => 'img', 'replace' => '<img class="LL" data-original="{param}" onload="if(this.width > 735) {this.width=735}" title="'.$title.'" alt="'.$title.'" />', 'parse' => false, 'validParam' => $imgValidator),
            array('tag' => 'color', 'replace' => '<span style="color: {option}">{param}</span>', 'option' => true, 'validOption' => $colorValidator),
            array('tag' => 'size', 'replace' => '<h2 style="font-size: {option}pt; line-height: {option}pt">{param}</h2>', 'option' => true, 'validOption' => $sizeValidator),
            ...
            array('tag' => 'code', 'replace' => '<div class="bbcmsg_code bbcmsg">{param}</div>'),
            array('tag' => 'info', 'replace' => '<div class="bbcmsg_info bbcmsg">{param}</div>'),
            array('tag' => 'warning', 'replace' => '<div class="bbcmsg_warning bbcmsg">{param}</div>'),
            array('tag' => 'note', 'replace' => '<div class="bbcmsg_note bbcmsg">{param}</div>'),
			array('tag' => 'message', 'replace' => '<div class="bbcmsg_message bbcmsg">{param}</div>'),
            array('tag' => 'download', 'replace' => '<div class="bbcmsg_download bbcmsg">{param}</div>'),
            array('tag' => 'success', 'replace' => '<div class="simple_box bbcmsg">{param}</div>')
        );
	...
	public function parseSmiles(){
	   global $tsCore;
		...
		$html = array();
        //
        $pre = '<img src="'.$tsCore->settings['url'].'/themes/default/images/smiles/';
        $end = '" align="absmiddle"/>';
		// SMILES DEFAULT
