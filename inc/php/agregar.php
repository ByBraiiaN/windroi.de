...
\*********************************/
	...
	if(is_numeric($action)){
		...
		if($tsPost > 0) {
			...
			//
			$smarty->assign("tsAviso",array('titulo' => 'Bien!', 'mensaje' => 'El post <b>'.$_POST['titulo'].'</b> fue agregado. '.(!$tsUser->is_admod && ($tsUser->permisos['gorpap'] == true || $tsCore->settings['c_desapprove_post'] == 1) ? 'Deber&aacute; esperar su aprobaci&oacute;n' : '').' ', 'but' => ''.(!$tsUser->is_admod && ($tsUser->permisos['gorpap'] == true || $tsCore->settings['c_desapprove_post'] == 1) ? 'P&aacute;gina principal' : 'Acceder al post').'', 'link' => ''.(!$tsUser->is_admod && ($tsUser->permisos['gorpap'] == true || $tsCore->settings['c_desapprove_post'] == 1) ? ''.$tsCore->settings['url'].'/posts/' : ''.$tsCore->settings['url'].'/posts/'.$tsCat['c_seo'].'/'.$tsPost.'/'.$tsCore->setSEO($_POST['titulo']).'.html').''));
		} elseif($tsPost == -1){
