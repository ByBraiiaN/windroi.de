...
\*********************************/
	...
	switch($action){
		...
		case 'feed-version':
			 /**
             * Versión a 10 de abril de 2016 *
             * PHPost Risus 1.2.6.000 *
             */
            $version_now = 'Risus 1.2.6.000';
            # ACTUALIZAR VERSIÓN
            if($tsCore->settings['version'] != $version_now){
			    db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `w_configuracion` SET version = \''.$version_now.'\', version_code = \'risus_1_2_6_000\' WHERE tscript_id = \'1\' LIMIT 1');
